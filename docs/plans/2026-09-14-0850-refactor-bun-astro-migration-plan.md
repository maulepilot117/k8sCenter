...
The fix is a `Sec-WebSocket-Protocol` credential, following the convention the Kubernetes apiserver uses for this same constraint. The client offers two subprotocols: `base64url.bearer.authorization.k8scenter.io.<token>`, base64url-encoded without padding so it is a valid RFC 6455 token — raw JWTs can contain characters that are not, and `=` padding is not — and a plain sentinel, `k8scenter.v1`. The server echoes **only the sentinel**; echoing the credential would move it into a response header and defeat the point. `frontend/server/ws-proxy.ts` decodes it into a real `Authorization` header on the outbound leg, and a genuine header still wins when one is present, so non-browser clients keep the original path.

A query parameter was implemented first and worked, but put the credential in a URL for the browser-to-proxy hop, where an ingress would log it. That form is gone; there is no fallback to it, because leaving one would preserve the exposure the change exists to close. The credential now appears in no URL on either leg, and nothing on this path logs a request URL.
...
KTD5. **Shared state stays module-scope signals, with bundler dedupe and an explicit sharing test.** Astro documents cross-island sharing via a shared module, and Rollup dedupes a common import into one chunk, so the existing pattern should survive. The failure mode is not Astro but duplicate copies of `preact` or `@preact/signals-core` resolving twice, after which a `computed()` built from one copy cannot see a `signal()` from the other. Pin `resolve.dedupe` for both, and prove sharing in U1 rather than at the end of U9. U14 showed the rule generalises beyond Preact: Vite's dev optimizer pre-bundled `@codemirror/state` twice — once as its own dep chunk, once inlined through `@codemirror/language`'s graph — and because CodeMirror's `Facet` and `Extension` values are identity-checked rather than duck-typed, `EditorState.create()` threw on extensions built from the other copy. **Any library whose API compares object identity rather than shape has this failure mode**, so the dedupe list is a standing obligation for each such dependency, not a one-off fix for signals. It also only ever fails in a browser: every gate stayed green through it. Governs R7.
...
| Trivy cannot parse `bun.lock` | Low | Release gate passes vacuously | **Measured in U10 and worse than assumed.** `trivy image` reports `Number of language-specific files num=0` for the built image and does not read `/app/bun.lock` — nor did it ever read `/app/deno.lock`, so the application-dependency half of the release gate has been vacuous since before this migration. `trivy fs` on the same file parses it (`type=bun`), so U11 adds a lockfile filesystem scan alongside the image scan. Text `bun.lock` is the supported format; `bun.lockb` is not |
...
| `bun.lock` is not copied into the runtime image | Medium | Trivy scans an empty package set and the release gate passes vacuously | U10 makes the copy. The assertion it was supposed to enable does not hold in image mode — see the row above — so `bun run check` gains a guard that the committed lockfile is the text one, and U11 owns the `trivy fs` scan that actually reads it |
...
5. ~~Re-evaluate the forced `--platform=linux/amd64` on the builder.~~ Resolved in U10: the builder now runs on `$BUILDPLATFORM` and never under emulation at all. Everything expensive there (`bun install`, `astro build`) emits architecture-independent JavaScript, and the one architecture-dependent artifact — the compiled launcher — is cross-compiled with `bun build --compile --target=bun-linux-<arch>`. That is strictly better than the Deno pin, which existed to dodge a QEMU ARM64 crash rather than to be correct.

   The x64 target defaults to `bun-linux-x64-baseline`, not plain `bun-linux-x64`. The latter is faster but requires AVX2 (Intel Haswell, 2013, and later) and aborts with "Illegal instruction" on older CPUs. k8sCenter installs onto clusters whose nodes the project does not choose, so the default is the target that always starts; `--build-arg BUN_COMPILE_TARGET=bun-linux-x64` takes the faster path when the operator knows their fleet. arm64 has no baseline variant.
6. Copy the text `bun.lock` into the final image, mirroring today's `deno.lock` copy, and assert that `bun.lockb` is never committed — `frontend/server/check-lockfile-is-text.ts`, wired into `bun run check`.

   The premise behind the copy did not survive measurement. `trivy image` reports `Number of language-specific files num=0` for the built image: it does not read `/app/bun.lock`, and it does not read `/app/deno.lock` out of the image this one replaces either — verified against the published `ghcr.io/maulepilot117/k8scenter-frontend:latest`. So the release gate's application-dependency half has been vacuous all along, and this is a pre-existing hole the migration surfaced rather than one it opened. `trivy fs` on the same file parses it correctly (`Target='bun.lock' type=bun`), so U11 adds a filesystem lockfile scan that actually reads it. The copy stays regardless: it costs nothing, it is the manifest an operator or a later Trivy would read, and removing it would quietly make things worse.
...
6. Add a `trivy fs` scan of `frontend/bun.lock` to `ci-release.yml`, alongside the existing image scan, and gate the release on it. U10 measured that `trivy image` does not read the lockfile out of the image — it reports zero language-specific files, for the Deno image as much as the Bun one — so this step is not a nicety: without it the frontend's npm dependency tree has never been scanned by anything. Assert the result is a non-empty package set, so the step cannot pass vacuously the way the image scan has been.
7. Add a CI check that every pinned GitHub Action is SHA-pinned and at least seven days old, enforced rather than asserted.
8. Leave the separate Node setup in `e2e.yml` alone — that serves Playwright, not the frontend.
...
- Covers AE4. The `trivy fs` lockfile scan reports a non-empty package set read from `bun.lock`. An empty set is a failure, not a pass — that is the whole point of the step.
...
**Outcome (2026-09-15).** Executed as two changes. The deletion is staged on `refactor/u12-remove-fresh-tree` and gated on the soak, which restarted on `sha-581b68f` at 13:30 UTC on 2026-09-15 and closes 48h later; every other gate is green. Four corrections to the unit as written, each found only on contact:

- **The file list is wrong about `frontend/components/`.** It reads as the Fresh tree's property alongside `frontend/islands/`. It is not: 107 distinct imports reach it from the Astro tree and none has a `src/` twin. It is shared infrastructure like `frontend/lib/` and stays. Deleting it takes the build with it.
- **Six files outside `routes/` and `islands/` still imported `fresh/runtime`** — `lib/resource-counts.ts`, `lib/useNotificationCrud.ts`, `components/k8s/PinToggle.tsx`, `components/mesh/GoldenSignals.tsx`, `components/ui/CodeMirrorEditor.tsx`, `components/ui/MonacoEditor.tsx`. Each has a `src/` twin and is referenced only from the Fresh tree. Left behind they would have failed the AE3 sweep.
- **Ten imports inside the surviving `frontend/components/` pointed at the Fresh tree**, eight of them taking a wizard's *form type* from `@/islands/X.tsx` while being rendered by `@/src/islands/X.tsx`. A type-only fork has no runtime footprint, so `check-no-duplicate-lib-state.ts` — written for exactly this hazard — could not see it. Worth a follow-up: that guard's premise is value imports.
- **Step 2b's list of tests to re-home was incomplete.** The two U7 parity tests it names were already frozen behind `existsSync` guards and needed nothing. Two others did break: `check-no-ws-dev-proxy_test.ts` used `vite.config.ts` as its positive control (now frozen verbatim in the test), and `check-no-duplicate-lib-state_test.ts` asserted on an `islands/` twin (re-pointed at the components twins).

**The KTD14 blind spot, and what replaced it.** Step 1b's spot-check compares the two trees, so it cancels out any defect both trees share and can only see what the port itself introduced. Running the placeholder/hydrated-root analysis over *both* trees found 16 distinct divergences of the same class as the two KTD14 caught — all inherited verbatim from Fresh, none visible to a cross-tree diff. They are a standing latent-bug backlog, not a parity failure, and fixing them would change rendering, which R3 requires this unit prove it has not done. `frontend/server/check-placeholder-root-parity.ts` (in `bun run check`) now baselines them and fails on any new one; the baseline is reconciled in both directions so it can only shrink.

---

## Operational Notes

**Rollback was a single Helm command through U11, and stayed one until U12 landed.** The Fresh tree is now deleted, so rolling *back* past U12 means deploying a pre-U12 `sha-` image rather than reverting a config flag; the command is the same, the target is a tag from before the cutover. Everything below described that window and is kept as the record of it. `ci-release.yml` builds the frontend and backend images independently under a path filter, so a frontend-only change never moves the backend tag. Rolling back is `helm upgrade` with `frontend.image.tag` pinned to the previous `sha-` image. Nothing forces it one-way: the backend API contract is untouched, the refresh-token cookie is backend-issued, and pins and saved views are server state. The couplings that could break this are browser storage and the one cookie the frontend issues itself, which is why KTD13 makes their parity a verified requirement rather than an assumption. Deploy by pinning the explicit `sha-` tag, never `latest`, so the rollback target is unambiguous.

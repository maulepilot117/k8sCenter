...
1. Port the four signal stores. These use `IS_BROWSER` at module-evaluation time to gate a synchronous `localStorage` read and an `effect()` subscription — a different idiom from the per-render guards in U9, and the one KTD5's correctness argument rests on. Note that `pin-store.ts` does not import `IS_BROWSER` at all, guarding through its own signals instead; confirm that is deliberate rather than normalising it.

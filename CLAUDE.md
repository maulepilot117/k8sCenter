...
- **Tailwind CSS utility-only.** No custom CSS class names in components/islands. Theme via CSS custom properties (`var(--accent)`, `var(--success)`, etc.) — no hardcoded color classes. Exception: shared multi-property effect primitives that have no single-utility equivalent live in `assets/styles.css` (`.card-lift`, `.stagger-in`, `.page-enter`, `.glass`, `.glass-elevated`, `.glass-scrim`, `.glass-bar`) — add new ones there, never inline `<style>` or per-component CSS.
...
**Status:** Web phases 1–14 complete. Mobile M1 + M2 + M3 + M4 complete. M5 in progress (PR-5a + PR-5b + PR-5c shipped: Settings + Sentry scaffold + SecureScreenMixin + a11y helpers; backend OIDC body-mode mobile-exchange with 1h refresh TTL cap; full mobile OIDC flow with PKCE + state + nonce + Universal Link callback. PR-5h + PR-5i shipped: WCAG 2.2 AA accessibility pass across M1–M4 surfaces; DataTable2 virtualization + performance baseline. PR-5j infrastructure shipped: App Store + Play Store metadata, `promote_ios` + `promote_android` Fastlane lanes, public-store promotion runbook, App Privacy questionnaire reference, `/privacy` web route, #282 mobile-config API rename. Public-store submission itself blocked on operator-side prereqs: Apple Developer enrollment, Play first-AAB manual upload, branded app icons, real-device screenshots, Sentry DSN, `https://kubecenter.io/privacy` going live).
...
**Backend resilience — shipped (arc closed, PRs #370–#377):** Native Go fuzzing across the parse seams (security/SSRF/auth-PHC/YAML/LogQL/secret-masking + CRD normalizers across 7 packages; found + fixed 2 real certmanager panics) and backend-wide goroutine panic recovery via the shared `internal/recoverutil` (`Go`/`Tick`/`Safe`). Closed by #377: `ClientFactory → kubernetes.Interface` seam + `FuzzSecretPipeline` (full HTTP-handler Secret fuzzing — Oracle A no-panic + Oracle D no-plaintext-leak). Conventions consolidated in `docs/solutions/backend-resilience-conventions.md`. No deferred items remain.

**Design system (2026-06-10):** Liquid Glass redesign — a single dark `liquid-glass` theme replaced the former 7-theme system on both platforms (ThemeSelector island and ThemePickerSheet removed; stale persisted theme ids fall back). Tokens live in `shared/themes/liquid-glass.json`, generated to CSS + Dart via `tools/theme-gen` (`make check-themes` parity gate unchanged). Glass is chrome-only — `.glass*` utilities on web, `GlassContainer` on mobile; data surfaces (tables, editors, terminals) stay solid. Spec: `plans/liquid-glass-redesign.md`.
...
**Mobile — shipped (M5 PR-5a):** Foundation primitives. Sentry crash reporting (opt-in, default off) with layered PII scrub via `lib/observability/pii_scrubber.dart`. SecureScreenMixin (Android FLAG_SECURE + iOS lifecycle blur cover) ready for PR-5d to wire on SecretDetailScreen. Settings screen with Sentry toggle + About (the theme picker shipped here was removed in the 2026-06-10 Liquid Glass redesign — single theme, nothing to pick). A11y test helpers (`expectMeetsAllGuidelines`, `a11yHarness`, `findSemanticsFor`) ready for PR-5h.
...
**Mobile — shipped (M5 PR-5c):** Full mobile OIDC flow. `/v1/auth/oidc/{providerID}/mobile-config` helper endpoint exposes authorization endpoint + clientId + scopes (no secret) so mobile builds the auth URL with client-generated PKCE + state + nonce. Mobile side: PKCE primitives, OIDCRepository, OIDCController with sealed state machine + cold-start re-entry via secure_storage, UniversalLinkListener (app_links), OIDCProviderButton, login screen integration. Closes #277 (displayName response duplication dropped). JSON wire key for the new endpoint follows the project's camelCase convention as of PR-5j (#282) — `clientID` (Go style) renamed to `clientId` (wire) before public-launch lock-in.

**Mobile — shipped (M5 PR-5j infrastructure):** Public-store launch infrastructure on the `feat/m5-pr-5j-public-store-launch` branch. Agent-executable deliverables: (a) Apple App Store metadata under `mobile/fastlane/metadata/en-US/` (name, subtitle 27/30 chars, keywords 92/100, description 2667/4000, release_notes, privacy_url, support_url, marketing_url, age_rating.json all NONE → 4+). (b) Google Play metadata under `mobile/fastlane/metadata/android/en-US/` (title, short_description 65/80, full_description 2690/4000, release_notes 484/500). (c) `promote_ios` + `promote_android` Fastlane lanes mirroring the existing `beta_*` lane structure — `promote_ios` pushes metadata + screenshots and submits the latest TestFlight build for review with `submission_information` export-compliance hash; `promote_android` track-promotes the latest Internal AAB to Production at 10% staged rollout with no rebuild. (d) `mobile/docs/APP_PRIVACY.md` — App Privacy questionnaire reference, Play Data Safety mapping, third-party services table, on-device PII scrub specifics, user rights, submission checklist. (e) Public-store promotion runbook appended to `mobile/docs/RELEASE.md` — promotion prerequisite tracker (9 gates), iOS 12-step + Android 10-step first-launch procedures, compressed 5-step recurring-release table, troubleshooting, rollback paths. (f) `frontend/routes/privacy.tsx` — SSR privacy page at `/privacy` mirroring APP_PRIVACY.md for end-users + store reviewers. (g) README.md `## Mobile App` section with App Store + Play Store badges (App Store URL placeholder until Apple assigns numeric ID). (h) Backend `MobileAuthConfig` JSON tag rename `clientID → clientId` (#282) — locks the API contract before public-store launch ossifies it; Go field name `ClientID` retained per Go style. Backend + mobile regression tests guard against accidental reverts. **Not shipped on this branch (operator-side prereqs):** Apple Developer Program enrollment, Play first-AAB manual upload, branded app icon PNGs, real-device screenshots, Sentry DSN provisioning, `https://kubecenter.io/privacy` hosting. Plan status flip on `plans/mobile-app.md` and `plans/mobile-app-m5-pr-sequence.md` deferred until both stores approve.

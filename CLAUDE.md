...
### Mobile (M1 + M2 complete)

- **Mobile M1 (PR-1a → PR-1g) — read-only oncall companion:** Flutter 3.41 / Dart 3.11 scaffold under `mobile/` with Riverpod 2.x, go_router, Dio, web_socket_channel, flutter_secure_storage, firebase_messaging, data_table_2, fl_chart. `lib/theme/kube_theme_builder.dart` derives Material 3 `ThemeData` + `KubeColors` extension from `themes.g.dart`; `make check-themes` enforces parity. Adaptive shell (phone drawer / tablet two-pane at 768px). Dio interceptor stack (`Cluster`, `CSRF`, `Auth`, `ErrorMapping`) + body-mode `/v1/auth/refresh` with `Completer<bool>` 401 dedupe (try/finally guard against synchronous-throw leak). `auth_repository.dart` mirrors `frontend/lib/auth.ts`; bootstrap exchanges persisted refresh token then loads `/v1/auth/me` for `UserInfo` + `RBACSummary`. Cluster pill + bottom-sheet picker; `activeClusterProvider` watched by every per-cluster provider so cluster switches auto-invalidate. Dashboard backed by `/v1/cluster/dashboard-summary`. 12 specialized resource screens (Pod, Deployment, Service, ConfigMap, Secret, Node + ReplicaSet, StatefulSet, DaemonSet, Ingress, PVC, Namespace) over `resourceListProvider` / `resourceGetProvider` (autoDispose family, CancelToken on switch); generic detail catch-all for everything else. `resource_detail_scaffold.dart` provides Overview/YAML/Events tabs (Events filters by `involvedObject.uid`, falls back to kind+name+namespace tuple). Pod log tail via `KubeWebSocketClient` (auth-in-band handshake, capped exponential backoff 1s→30s, fatal-error sentinel, 5000-line ring buffer, dropped-count banner, pause/resume/clear). Notification feed over `/v1/notifications` with severity-tinted rows + unread badge + deep-links. FCM device registration (silent fall-through when Firebase config absent). Deep links: `k8scenter://cluster/<id>/<Kind>/<ns>/<name>` (custom scheme) + `https://<allowlisted-host>/m/cluster/...` (Universal Link). CI: `.github/workflows/mobile-ci.yml` runs `flutter analyze && flutter test` + theme parity; `deploy_check` probes secrets and conditionally fires `deploy_ios` / `deploy_android` Fastlane lanes (`match` + TestFlight; Play Internal). Helm `well-known.yaml` serves `apple-app-site-association` + `assetlinks.json` from a ConfigMap, gated on `mobile.universalLinkDomain`. **OIDC mobile deferred** — cookie-exchange path doesn't work in in-app browsers; body-mode flow needs a backend endpoint, tracked as follow-up.
- **Mobile M2 (PR-2a + PR-2b) — write actions + YAML editor:** `mobile/lib/api/resource_actions.dart` is a 1:1 Dart port of `frontend/lib/action-handlers.ts` (same `ActionId` enum, `actionsByKind` / `actionVerbMap`, `getVisibleActions` / `getActionMeta` / `executeAction`); `permissions.dart` ports `canPerform` with strict cluster-scoped default + opt-in `allowAnyNamespaceFallback` for All-Namespaces list views. `confirm_sheet.dart` mirrors `ConfirmDialog.tsx`: nullable `typeToConfirm` gates `FilledButton`, normalizes whitespace + ZWSP/ZWNJ/ZWJ/BOM before matching. `resource_actions_button.dart` orchestrates: ActionSheet → ScaleSheet/ConfirmSheet → `executeAction` → snackbar → `ref.invalidate(resourceGetProvider(...) + resourceListProvider)`. **Cluster pinning everywhere a write fires:** `_pinnedCluster` snapshotted at sheet-open / picker-build / controller-construction; mismatch with `activeClusterProvider` aborts with snackbar. `_executing` gate prevents double-tap. Per-action `Options(receiveTimeout: ...)` bumps delete to 90s for long-grace pods; `dioProvider` adds 30s `sendTimeout` for multi-MB ConfigMap uploads. `actionsByKind` covers all 12 specialized kinds + namespaces (cluster-scoped) — `_resourceBase` helper omits namespace segment when empty. **Suspend toggle** refreshes `resourceGetProvider` before send and aborts if state diverged from snapshot, preserving operator intent against concurrent mutation. **Rollback picker** at `/clusters/:cluster/workloads/deployments/:ns/:name/rollback` lists prior ReplicaSets owned by the deployment (filtered by `ownerReferences[*].uid`, sorted newest-first by `kRevisionAnnotation = 'deployment.kubernetes.io/revision'`, de-duped, current revision marked + tap-disabled). **YAML editor** (`yaml_apply_controller.dart` + `yaml_editor_panel.dart`): `AutoDisposeFamilyNotifier<YamlApplyState, YamlApplyKey>` with idle/validating/validated/applying/applied/failed state machine. `_post` unwraps the `{data: ...}` envelope and dispatches via `isApply: bool` because validate returns `{documents, valid}` while apply returns `{results, summary}` — `ApplyResponse.fromValidate` adapts docs into the shared display model. Editor: read-only `JsonEncoder.withIndent` view + `TextField(maxLines: null)` edit mode (no `code_text_field` dep — functional-not-pretty per plan). **Secret-data destruction defense:** `stripSensitiveDataFields: true` removes `data`/`stringData` from the editor seed for Secrets — backend masks those values to `"****"` on GET, so SSA on an edited Secret would otherwise overwrite real credentials with the mask string. `ResourceDetailScaffold` gains `editableYaml` + `applyKey` (asserted in pair); ConfigMap and Secret opt in. Generic `e.toString()` snackbar leak sanitized; `ApiError`-only error surfaces. 184 tests pass (`flutter analyze` clean, `make check-themes` in sync). M2 (write actions + YAML edit for ConfigMap/Secret) complete; M3 wizards next.

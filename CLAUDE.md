...
- **Mobile M2 (PR-2b, M2 complete):** Delete (type-to-confirm) on every M1 specialized kind + rollback (revision picker) for deployments + ConfigMap/Secret YAML edit through `/v1/yaml/apply`. `actionsByKind` now spans all 12 specialized M1 kinds plus generic-detail fallback (services, configmaps, secrets, replicasets, ingresses, persistentvolumeclaims, namespaces all gain delete; deployments re-add rollback). `executeAction` URL builder uses a `_resourceBase` helper that omits the namespace segment for cluster-scoped resources, so namespace delete builds `/api/v1/resources/namespaces/<name>` correctly. **Rollback picker** — `mobile/lib/features/resources/rollback_picker_screen.dart` is a full-screen route at `/clusters/:cluster/workloads/deployments/:ns/:name/rollback` listing prior ReplicaSet revisions filtered by `ownerReferences[*].uid` matching the deployment, sorted newest revision first by the `deployment.kubernetes.io/revision` annotation. Surfaces change-cause when set. `ResourceActionsButton` routes `ActionId.rollback` to this screen via `context.push` instead of opening a confirm sheet directly. **YAML editor** — `mobile/lib/api/yaml_apply_controller.dart` is `AutoDisposeFamilyNotifier<YamlApplyState, YamlApplyKey>` with the same idle/validating/validated/applying/applied/failed state machine `frontend/lib/yaml-apply.ts:useYamlApply` carries. Apply success calls `ref.invalidate(resourceGetProvider(...))` so the read-only YAML and Overview tabs reflect the new state on next render. `mobile/lib/widgets/yaml_editor_panel.dart` toggles between read-only `JsonEncoder.withIndent` view + Edit button and edit-mode `TextField(maxLines: null)` (no syntax highlighting — `code_text_field` is not in pubspec; functional-not-pretty per the plan's risk mitigation). Validate / Apply buttons drive the controller; result panel shows summary chips (`N created`, `N configured`, `N unchanged`, `N failed`) and per-row errors. `ResourceDetailScaffold` gains `editableYaml` + `applyKey` parameters (asserted in pair via `assert()` in the constructor); ConfigMap and Secret detail opt in. Secret-mode editor renders a "Secret data is base64. Edits go through /v1/yaml/apply server-side-apply." banner above the editor. **Tests** (14 new): `test/api/yaml_apply_controller_test.dart` (state-machine transitions, invalidate-on-apply, error path), `test/widgets/yaml_editor_panel_test.dart` (Edit toggle, Validate POST, dry-run panel, Cancel restore), `test/features/resources/rollback_picker_screen_test.dart` (revision filtering by uid, newest-first sort, mismatched-uid skip, empty state, tap → ConfirmSheet). 184 tests pass; `flutter analyze` clean. M2 (write actions + YAML editor for ConfigMap/Secret) is now complete.

...
/**
 * Convert a nested form state (dot-notation keyed) to a nested object.
 * e.g. { "issuerRef.name": "letsencrypt" } => { issuerRef: { name: "letsencrypt" } }
 */
...
/**
 * Convert a nested object to flat dot-notation keyed form state.
 */
export function nestedToFlat(
  ...
  for (const [key, value] of Object.entries(obj)) {
    ...
    if (
      ...
      Object.assign(result, nestedToFlat(value as Record<string, unknown>, path));
    } else {
      result[path] = value;
    }
  }

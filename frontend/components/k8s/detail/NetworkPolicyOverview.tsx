...
function PeerBadge({ peer }: { peer: NetworkPolicyPeer }) {
  if (peer.ipBlock) {
    return (
      <span class="inline-flex items-center gap-1 rounded bg-purple-100 dark:bg-purple-900/30 px-2 py-0.5 text-xs font-mono text-purple-800 dark:text-purple-300">
  ...
  if (parts.length === 0) return <span class="text-xs text-slate-400">-</span>;

  return (
    <span class="inline-flex items-center rounded bg-blue-100 dark:bg-blue-900/30 px-2 py-0.5 text-xs text-blue-800 dark:text-blue-300">
      {parts.join(" / ")}
    </span>
  );
}
...
function RuleRow(
  ...
  return (
    <tr class="border-b border-slate-200 dark:border-slate-700 last:border-0">
      <td class="py-2 px-3 text-xs text-slate-600 dark:text-slate-300 align-top">
        {hasNoPeers
          ? (
            <span class="inline-flex items-center rounded bg-amber-100 dark:bg-amber-900/30 px-2 py-0.5 text-xs text-amber-800 dark:text-amber-300">
      ...
      <td class="py-2 px-3 text-xs font-mono text-slate-600 dark:text-slate-300 align-top">
            ...
            <table class="w-full text-left">
              <thead>
                <tr class="bg-slate-50 dark:bg-slate-800/50">
                  <th class="py-1.5 px-3 text-xs font-medium text-slate-500 dark:text-slate-400">

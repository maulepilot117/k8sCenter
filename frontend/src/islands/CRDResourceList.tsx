...
function getReadyStatus(item: CRDItem): "Ready" | "NotReady" | "Unknown" {
...
export default function CRDResourceList({ group, resource }: Props) {
  ...
  useEffect(() => {
    ...
    const fetchInstances = async () => {
      ...
      try {
        ...
        continueToken.value = (
          res as { metadata?: { continue?: string } }
        ).metadata?.continue;
      } catch {
        items.value = [];
  ...
  const loadMore = async () => {
    ...
    try {
      ...
      const nsPath = ns && ns !== "all" ? `/${ns}` : "";
      const res = await apiGet<CRDItem[]>(
        `/v1/extensions/resources/${group}/${resource}${nsPath}?limit=100&continue=${encodeURIComponent(
          continueToken.value,
        )}`,
      );
    ...
    <div class="flex flex-col h-full">
          ...
          {(
            ...
            <button
              ...
              style={{
                ...
                cursor: "pointer",
                background:
                  statusFilter.value === val ? "var(--accent)" : "transparent",
                color:
      ...
      >
        ...
        <div
          style={{
            display: "grid",
            gridTemplateColumns: crdGridTemplate(isNamespaced, printerColumns),
            gap: "12px",
            padding: "11px 18px",
        ...
        {filtered.map((item) => {
          ...
          const ns = item.metadata.namespace;
          const detailNs = ns ?? "_";
          const detailHref = `/extensions/${group}/${resource}/${detailNs}/${item.metadata.name}`;

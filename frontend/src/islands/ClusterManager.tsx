...
export default function ClusterManager() {
  ...
  async function addCluster() {
    ...
    try {
      ...
      error.value =
        err instanceof Error ? err.message : "Failed to register cluster";
    } finally {
      saving.value = false;
    }
  }
          ...
          >
            <span
              style={{
                ...
                flexShrink: 0,
                background:
                  c.status === "connected" ? "var(--success)" : "var(--error)",
              }}
            />
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
      ...
      >
        <span style={{ fontSize: "13px", color: "var(--text-muted)" }}>
          {clusters.value.length} cluster
          {clusters.value.length !== 1 ? "s" : ""} registered
        </span>
        <Button
          type="button"
          variant="primary"
      ...
      {clusters.value.length === 0 ? (
        ...
        <ResourceTable columns={columns} rows={rows} chevron={false} />
      )}
    </div>
  );
}

...
function getResourceHref(kind: string, ns: string, name: string): string {
  ...
  return (
    kindRoutes[kind] ??
    `/resources/${encodeURIComponent(kind)}/${encodeURIComponent(ns)}/${encodeURIComponent(
      name,
    )}`
  );
}
...
export default function NamespaceTopology({
  ...
  if (!graph.value || graph.value.nodes.length === 0) {
    return (
      <div class="flex items-center justify-center rounded-lg border border-border-primary bg-bg-surface p-12">
        <p class="text-text-secondary">No resources found in this namespace.</p>
      </div>
    );
  }
          ...
          {layoutNodes.value.map((node) => {
            ...
            return (
              <g
                ...
                onClick={() => {
                  selectedNode.value =
                    selectedNode.value === node.id ? null : node.id;
                }}
              >
                <rect
                  width={NODE_WIDTH}
        ...
        >
          ...
          <div class="space-y-4 p-4">
            ...
            <div class="space-y-2 pt-2">
              ...
              {selected.kind === "Pod" && (
                <a
                  href={`/observability/logs?namespace=${encodeURIComponent(
                    selected.namespace,

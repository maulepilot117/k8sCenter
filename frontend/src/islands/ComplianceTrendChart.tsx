...
export default function ComplianceTrendChart() {
  ...
  const current =
    realPoints.length > 0 ? realPoints[realPoints.length - 1].score! : null;
  const first = realPoints.length > 0 ? realPoints[0].score! : null;
  const delta = current !== null && first !== null ? current - first : null;
  const best =
    realPoints.length > 0 ? Math.max(...realPoints.map((p) => p.score!)) : null;
  const worst =
    realPoints.length > 0 ? Math.min(...realPoints.map((p) => p.score!)) : null;

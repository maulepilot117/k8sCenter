...
export default function GatewayAPIDashboard() {
  ...
  async function fetchList(kind: GatewayResourceKind) {
    ...
    try {
      ...
      // The Fresh original suppressed this with a deno-lint-ignore
      // directive Biome cannot read. The cast is carried over verbatim
      // under the parity mandate; the suppression below must stay on the
      // line directly above the diagnostic for Biome to honour it.
        ...
        >
          ...
          {listData.value &&
            (() => {
              ...
              const visibleCount =
                activeKind.value === "gatewayclasses"
                  ? totalItems
                  : filterByNamespace(
...
function filterBySearch<T extends { name: string; namespace?: string }>(
  items: T[],
  query: string,
): T[] {
  ...
  switch (kind) {
    case "gatewayclasses":
      ...
      return renderGatewayClassesTable(
        filterBySearch(data.items as GatewayClassSummary[], query),
      );
    case "gateways":
...
function renderRoutesTable(kind: GatewayResourceKind, items: RouteSummary[]) {
  if (items.length === 0) {
    return (
      <EmptyTable

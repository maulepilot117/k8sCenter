...
function FailedCheck({
  ...
  const iconColor =
    result.status === "fail" ? "var(--error)" : "var(--warning)";
  ...
  return (
    ...
    >
      ...
      >
        <span style={{ color: iconColor, fontWeight: 700, fontSize: "14px" }}>
          {icon}
        </span>
        <span
          style={{
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
      ...
      <div>
        ...
        {failed.length === 0 ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
            {failed.map((r) => (
              <FailedCheck key={r.ruleName} result={r} namespace={namespace} />
            ))}
          </div>
        )}
      </div>
      ...
      <div>
        ...
        {passed.length === 0 ? (
          ...
          >
            {passed.map((r) => (
              <PassedCheck key={r.ruleName} result={r} />
            ))}
          </div>
        )}
      </div>

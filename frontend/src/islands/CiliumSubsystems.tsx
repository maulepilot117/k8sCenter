...
export default function CiliumSubsystems() {
  const { data, loading, error, lastFetchedAt } =
    usePoll<CiliumSubsystemsResponse>("/v1/networking/cilium/subsystems", {
      interval: 60_000,
  ...
  if (!resp?.configured) {
    return (
      <Card title="Cilium Subsystems" class="opacity-50">
        <p class="text-sm text-text-muted">Subsystem data not available.</p>
      </Card>
    );
  }
          ...
          {mesh.enabled ? (
            <div class="space-y-1.5">
              <div class="flex justify-between text-sm">
                ...
                <StatusBadge
                  label={
                    mesh.engine.charAt(0).toUpperCase() + mesh.engine.slice(1)
                  }
                  tone="ok"
                />
              </div>

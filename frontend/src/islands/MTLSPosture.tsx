...
function WorkloadRow({ w }: { w: WorkloadMTLS }) {
  ...
  if (w.source === "default") {
    defaultSubLine =
      w.mesh === "linkerd" ? "Default-on (Linkerd)" : "No explicit policy";
  }
...
export default function MTLSPosture() {
  ...
  const meshedCards = allCards.filter(
    (c) => classifyCard(c) !== "unmeshed-only",
  );
  const unmeshedOnlyCards = allCards.filter(
    (c) => classifyCard(c) === "unmeshed-only",
  );

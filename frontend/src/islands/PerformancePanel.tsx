...
export default function PerformancePanel({
  ...
  async function loadMetrics() {
    ...
    const effectiveName =
      kind === "nodes" && nodeInstance ? nodeInstance : name;
  ...
  if (!QUERIES[kind]) {
    return (
      <div class="p-12 text-center text-sm text-text-muted">
        ...
        <p class="mt-1">Metrics are not configured for this resource type.</p>
      </div>
    );
  }
...
function MiniChart({
  ...
  const points = values.map((v, i) => {
    ...
    const y =
      height - padding - ((v.value - minVal) / range) * (height - padding * 2);
    return `${x},${y}`;
  });

...
function podStatusInfo(pod: PodInfo): { text: string; color: string } {
...
function aggregateResources(containers: PodInfo["spec"]["containers"]): {
  cpuRequest: string;
  cpuLimit: string;
  memRequest: string;
  memLimit: string;
  ...
  return {
    ...
    cpuPercent:
      totalCpuLim > 0 ? Math.min(100, (totalCpuReq / totalCpuLim) * 100) : 0,
    memPercent:
      totalMemLim > 0 ? Math.min(100, (totalMemReq / totalMemLim) * 100) : 0,
    hasResources,
  };
}
  ...
  async function fetchPods() {
    ...
    try {
      const res = await apiGet<PodInfo[]>(
        `/v1/resources/pods/${namespace}?labelSelector=${encodeURIComponent(
          labelSelector,
        )}`,
      );

...
function getHealthStatus(
  ...
  if (kind === "node") {
    ...
    const ready = nodeRes.status?.conditions?.find((c) => c.type === "Ready");
...
export default function ClusterTopology() {
  ...
  const tooltip = useSignal<{ nodeId: string; x: number; y: number } | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const extractArr = <T,>(res: PromiseSettledResult<{ data: T[] }>): T[] =>
  ...
  useEffect(() => {
    ...
    const placeRow = (
      ...
      items.forEach((item, i) => {
        const base =
          kindAbbr[item.kind] ?? item.kind.substring(0, 2).toUpperCase();
    ...
    for (let i = 0; i < k8sWorkloads.length; i++) {
      ...
      const placed = topoNodes.find(
        (n) =>
          n.id === `wl-${wl.kind}-${wl.metadata.namespace}-${wl.metadata.name}`,
      );
      if (placed) {
    ...
    for (const pod of k8sPods) {
      for (const vol of pod.spec?.volumes ?? []) {
        if (vol.persistentVolumeClaim?.claimName) {
          const pvcId = `pvc-${pod.metadata.namespace}-${vol.persistentVolumeClaim.claimName}`;
          const podId = `pod-${pod.metadata.namespace}-${pod.metadata.name}`;
                            ...
                            {rel.items.map((item) => (
                              <span
                                ...
                                style={{
                                  padding: "1px 6px",
                                  borderRadius: "3px",
                                  background: `color-mix(in srgb, ${rel.color} 10%, transparent)`,
                                  border: `1px solid color-mix(in srgb, ${rel.color} 20%, transparent)`,
                                  fontSize: "10px",
                                  fontFamily: "var(--font-mono)",
                                  color: "var(--text-secondary)",
                                  maxWidth: "140px",

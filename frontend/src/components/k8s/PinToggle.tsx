...
export function PinToggle({
  ...
  const repin = async () => {
    ...
    try {
      ...
      if (preferenceReason(err) === "already_pinned") {
        ...
        failure.value = `Could not re-pin, and the old pin may already be gone. ${describe(
          err,
        )}`;
        await loadPins();
      }
  ...
  if (unavailable) {
    ...
    action = unpin;
    ariaLabel = `Unpin ${displayKind} ${name}`;
    hint = `A different ${displayKind} now carries this name. The pin still records the object you pinned.`;

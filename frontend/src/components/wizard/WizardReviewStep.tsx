...
export function WizardReviewStep({
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      applyError.value =
        err instanceof Error ? err.message : "Failed to apply resource";
    } finally {
  ...
  if (applyResult.value) {
    ...
    const detailPath =
      firstResult && !firstResult.error
        ? `${detailBasePath}/${encodeURIComponent(firstResult.namespace ?? "")}/${encodeURIComponent(
            firstResult.name,
          )}`
        : null;
            ...
            {result.results.map((r, i) => (
              <div
                ...
                class={`flex items-center gap-2 ${
                  r.error ? "text-danger" : "text-text-secondary"
                }`}
              >
                {r.error ? (
                  <svg

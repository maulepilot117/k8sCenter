...
import { rejectMalformed } from "./dispatch.ts";
 ...
 * malformed-request guard, and the /ws/* bridge) into `astro dev`'s own
 * Vite dev server (R18).
 *
 ...
 * install our headers/guard/ws-426 middleware at the very front of Vite's
 * middleware stack (calling `server.middlewares.use` directly inside
 * `configureServer`, rather than returning a post-hook function, registers
 * it *before* Vite's own internal middlewares -- so it also covers static
 * assets and Astro's own dev rendering, not just this plugin's own routes),
 * and attach ws-proxy.ts's identical 'upgrade' handler to the httpServer
 * Vite already created. U1 proved this exact shape end to end: dev and the
 * built server run through the same allowlist, traversal guard, queueing,
 * and exec Bearer-header logic.
 *

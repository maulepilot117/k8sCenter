...
export default function UserWizard({ onClose }: { onClose?: () => void }) {
  ...
  return (
    <WizardShell
      ...
      onCancel={close}
      onBack={goBack}
      onNext={created.value
        ? close
        : (currentStep.value === 0 ? goNext : handleSubmit)}
      nextLabel={created.value
        ? "Done"
        : (currentStep.value === 0 ? "Continue" : "Create User")}
    >
      {created.value
        ? (
          ...
          <div class="mx-auto max-w-lg">
            ...
            {currentStep.value === 1 && (
              <div class="space-y-4">
                ...
                {submitError.value && (
                  <ErrorBanner message={submitError.value} />
                )}
              </div>
            )}
          </div>

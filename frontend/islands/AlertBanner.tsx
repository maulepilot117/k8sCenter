...
export default function AlertBanner() {
  ...
  return (
    <div class="bg-danger-dim border-b border-danger px-4 py-2">
      <div class="flex items-center justify-between max-w-7xl mx-auto">
        <a
          href="/alerting"
          class="flex items-center gap-2 text-sm text-danger hover:text-danger"
        >
          <svg
            class="w-4 h-4"
            fill="none"
          ...
          {critical > 0 && (
            <span class="bg-danger-dim text-danger text-xs px-1.5 py-0.5 rounded-full">
              {critical} critical
            </span>
          )}
          {warning > 0 && (
            <span class="bg-warning-dim text-warning text-xs px-1.5 py-0.5 rounded-full">
              {warning} warning
            </span>
          )}
        </a>

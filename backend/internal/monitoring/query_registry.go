...
var Registry = map[string]QueryDef{
	...
	"pods/cilium-drops": {
		...
		Template:      `sum(rate(hubble_drop_total{reason="Policy denied",destination=~"{{.NamespaceRegex}}/{{.NameRegex}}"}[5m]))`,
		RequiredVerbs: []string{"list"},
		RequiredGVR:   "pods",
	...
	"deployments/cpu": {
		...
		Template:      `sum(rate(container_cpu_usage_seconds_total{namespace="{{.Namespace}}",pod=~"{{.NameRegex}}-.*",container!=""}[5m]))`,
		RequiredVerbs: []string{"list"},
	...
	"deployments/memory": {
		...
		Template:      `sum(container_memory_working_set_bytes{namespace="{{.Namespace}}",pod=~"{{.NameRegex}}-.*",container!=""}) / 1024 / 1024`,
		RequiredVerbs: []string{"list"},
		RequiredGVR:   "deployments.apps",
	...
	"deployments/network-rx": {
		...
		Template:      `sum(rate(container_network_receive_bytes_total{namespace="{{.Namespace}}",pod=~"{{.NameRegex}}-.*"}[5m])) / 1024`,
		RequiredVerbs: []string{"list"},
		RequiredGVR:   "deployments.apps",
	...
	"deployments/network-tx": {
		...
		Template:      `sum(rate(container_network_transmit_bytes_total{namespace="{{.Namespace}}",pod=~"{{.NameRegex}}-.*"}[5m])) / 1024`,
		RequiredVerbs: []string{"list"},
		RequiredGVR:   "deployments.apps",
	...
	"deployments/cpu-request": {
		...
		Template:      `sum(kube_pod_container_resource_requests{namespace="{{.Namespace}}",pod=~"{{.NameRegex}}-.*",resource="cpu"})`,
		RequiredVerbs: []string{"list"},
		RequiredGVR:   "deployments.apps",
	...
	"deployments/memory-request": {
		...
		Template:      `sum(kube_pod_container_resource_requests{namespace="{{.Namespace}}",pod=~"{{.NameRegex}}-.*",resource="memory"}) / 1024 / 1024`,
		RequiredVerbs: []string{"list"},
		RequiredGVR:   "deployments.apps",
	...
	"nodes/cpu": {
		Slug:          "nodes/cpu",
		Template:      `100 - (avg(rate(node_cpu_seconds_total{mode="idle",instance=~"{{.NameRegex}}"}[5m])) * 100)`,
		RequiredVerbs: []string{"list"},
		RequiredGVR:   "nodes",
	...
	"nodes/memory": {
		Slug:          "nodes/memory",
		Template:      `100 * (1 - node_memory_MemAvailable_bytes{instance=~"{{.NameRegex}}"} / node_memory_MemTotal_bytes{instance=~"{{.NameRegex}}"})`,
		RequiredVerbs: []string{"list"},
		RequiredGVR:   "nodes",
	...
	"nodes/load": {
		Slug:          "nodes/load",
		Template:      `node_load5{instance=~"{{.NameRegex}}"}`,
		RequiredVerbs: []string{"list"},
		RequiredGVR:   "nodes",
	...
	"nodes/network-rx": {
		...
		Template:      `sum(rate(node_network_receive_bytes_total{instance=~"{{.NameRegex}}",device!~"veth.*|cali.*|lxc.*|cilium.*"}[5m])) / 1024 / 1024`,
		RequiredVerbs: []string{"list"},
		RequiredGVR:   "nodes",
	...
	"nodes/network-tx": {
		...
		Template:      `sum(rate(node_network_transmit_bytes_total{instance=~"{{.NameRegex}}",device!~"veth.*|cali.*|lxc.*|cilium.*"}[5m])) / 1024 / 1024`,
		RequiredVerbs: []string{"list"},
		RequiredGVR:   "nodes",
	...
	"nodes/disk-read": {
		...
		Template:      `sum(rate(node_disk_read_bytes_total{instance=~"{{.NameRegex}}"}[5m])) / 1024 / 1024`,
		RequiredVerbs: []string{"list"},
		RequiredGVR:   "nodes",
	...
	"nodes/disk-write": {
		...
		Template:      `sum(rate(node_disk_written_bytes_total{instance=~"{{.NameRegex}}"}[5m])) / 1024 / 1024`,
		RequiredVerbs: []string{"list"},
		RequiredGVR:   "nodes",
	...
	"namespaces/cilium-drops": {
		...
		Template:      `sum(rate(hubble_drop_total{reason="Policy denied",destination=~"{{.NameRegex}}/.*"}[5m]))`,
		RequiredVerbs: []string{"list"},
		RequiredGVR:   "pods",
	...
	"networkpolicies/cilium-forwarded": {
		...
		Template:      `sum(rate(hubble_flows_processed_total{verdict="FORWARDED",destination=~"{{.NamespaceRegex}}/.*"}[5m]))`,
		RequiredVerbs: []string{"list"},
	...
	"networkpolicies/cilium-dropped": {
		...
		Template:      `sum(rate(hubble_flows_processed_total{verdict="DROPPED",destination=~"{{.NamespaceRegex}}/.*"}[5m]))`,
		RequiredVerbs: []string{"list"},
		RequiredGVR:   "networkpolicies.networking.k8s.io",
	...
	"networkpolicies/policy-denied": {
		...
		Template:      `sum(rate(hubble_drop_total{reason="Policy denied",destination=~"{{.NamespaceRegex}}/.*"}[5m]))`,
		RequiredVerbs: []string{"list"},
		RequiredGVR:   "networkpolicies.networking.k8s.io",
	...
	"networkpolicies/tcp-syn": {
		...
		Template:      `sum(rate(hubble_tcp_flags_total{flag="SYN",destination=~"{{.NamespaceRegex}}/.*"}[5m]))`,
		RequiredVerbs: []string{"list"},
		RequiredGVR:   "networkpolicies.networking.k8s.io",
	...
	"ciliumnetworkpolicies/policy-verdicts": {
		...
		Template:      `sum(rate(hubble_policy_verdict_total{destination=~"{{.NamespaceRegex}}/.*"}[5m])) by (action)`,
		RequiredVerbs: []string{"list"},
		RequiredGVR:   "ciliumnetworkpolicies.cilium.io",
	...
	"services/endpoint-cpu": {
		...
		Template:      `sum(rate(container_cpu_usage_seconds_total{namespace="{{.Namespace}}",pod=~"{{.NameRegex}}.*",container!=""}[5m])) by (pod)`,
		RequiredVerbs: []string{"list"},

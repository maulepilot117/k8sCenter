...
// AccessChecker.CanAccessGroupResource call reaches this per request, so a
// handler that (bug) cached the first identity's decision, or that used
// CanAccess (which short-circuits to allow in predicate-fake mode — brief
// A1), would make both identities come back identical. It satisfies the
...
func TestCapabilities_UnreachableIsNotUnsupported(t *testing.T) {
	...
	cap := buildCapability(findOp(t, "yaml.apply"), false, "", reach, nil, false, nil, nil, now)
	if !cap.PlatformSupported {
...
func TestCapabilities_ReasonCodesAreClosed(t *testing.T) {
	...
	scenarios := []struct {
		...
		{name: "local ok", isLocal: true, reach: reachabilityResult{reachable: &trueVal, observedAt: now}, discoveryPresent: &trueVal, authorized: &trueVal},
		{name: "local forbidden", isLocal: true, reach: reachabilityResult{reachable: &trueVal, observedAt: now}, discoveryPresent: &trueVal, authorized: &falseVal},
		{name: "local authz unknown", isLocal: true, reach: reachabilityResult{reachable: &trueVal, observedAt: now}, discoveryPresent: &trueVal, authErr: errors.New("SAR failed")},
		{name: "local discovery missing", isLocal: true, reach: reachabilityResult{reachable: &trueVal, observedAt: now}, discoveryPresent: &falseVal, authorized: &trueVal},
		{name: "local discovery unavailable", isLocal: true, reach: reachabilityResult{reachable: &trueVal, observedAt: now}, discoveryUnavailable: true, authorized: &trueVal},
		{name: "remote ok", isLocal: false, reach: reachabilityResult{reachable: &trueVal, observedAt: now}, discoveryPresent: &trueVal, authorized: &trueVal},
		{name: "reachable but authorized never computed", isLocal: true, reach: reachabilityResult{reachable: &trueVal, observedAt: now}, discoveryPresent: &trueVal},
		{name: "remote unreachable", isLocal: false, reach: reachabilityResult{reachable: &falseVal, observedAt: now}},
		{name: "remote stale", isLocal: false, reach: reachabilityResult{observedAt: now}},
		{name: "remote cluster_unknown", isLocal: false, globalReason: ReasonClusterUnknown},
		{name: "remote credentials_invalid", isLocal: false, globalReason: ReasonCredentialsInvalid},
		{name: "remote db_unavailable", isLocal: false, globalReason: ReasonDBUnavailable},
	}
	...
	for _, sc := range scenarios {
		t.Run(sc.name, func(t *testing.T) {
			for _, op := range capabilityOperations {
				...
				if !op.supportedFor(sc.isLocal) && cap.ReasonCode != ReasonUnsupportedPlatform {
					t.Errorf("operation %q is not platform-supported for isLocal=%v but reasonCode = %q; want %q",
						op.ID, sc.isLocal, cap.ReasonCode, ReasonUnsupportedPlatform)
				}
				if op.supportedFor(sc.isLocal) && cap.ReasonCode == ReasonUnsupportedPlatform {
					t.Errorf("operation %q IS platform-supported for isLocal=%v but reasonCode = unsupported_platform", op.ID, sc.isLocal)
				}
			}
		})
	}
